...
public class MyGUI extends JFrame {
    ...
    // Farbvariablen werden hier einmal definiert, sonst muss jedes Mal, wenn eine Farbe verwendet wird, ein neues Farbobjekt erstellt werden.
    ...
    // Font-Variablen werden hier einmal definiert, sonst muss jedes Mal, wenn eine Farbe verwendet wird, ein neues Font-Obj erstellt werden.
    ...
    public MyGUI(MyVars myVars, Config config) {
        ...
        this.myVars = myVars;
        this.config = config;
        speechRecognition = SpeechRecognition.getInstance(); // "this." nicht benötigt, da ".getInstance()" benutzt wird.
        initComponents();
    }

    // Methode gestohlen von einem Spielprojekt, an dem ich arbeite
    // Link für das Github-Repositorium: https://github.com/TheOriginalAn3/Some2DGame/blob/master/src/main/ImageHandler.java
    ...
    private void initComponents() {
        ...
        /*----------------------------------------------------------------------------------------------------------------------------------------*/
        // Ich habe keine Ahnung, was das macht 😅. Ich habe die GUI mithilfe von NetBeans Scene Builder erstellt.
        // Ich denke, dies ist nur für den Abstand des Textes innerhalb des Bereichs "Right Words".
        // Ich denke dies fügt auch die Beschriftungen als untergeordnete Komponenten zum jPanelRightWords hinzu
        ...
        /*----------------------------------------------------------------------------------------------------------------------------------------*/
        // Ich habe keine Ahnung, was das macht 😅. Ich habe die GUI mithilfe von NetBeans Scene Builder erstellt.
        // Ich denke, dies ist nur für den Abstand des Textes innerhalb des Bereichs "Wrong Words".
        // Ich denke dies fügt auch die Beschriftungen als untergeordnete Komponenten zum jPanelWrongWords hinzu
        ...
        /*----------------------------------------------------------------------------------------------------------------------------------------*/
        // Ich habe keine Ahnung, was das macht 😅. Code hier wurde von NetBeans Scene Builder erstellt als ich die GUI zum ersten Mal Erstellt habe.
        ...
        /*----------------------------------------------------------------------------------------------------------------------------------------*/
        // Ich habe keine Ahnung, was das macht 😅. Code hier wurde von NetBeans Scene Builder erstellt als ich die GUI zum ersten Mal Erstellt habe.
        // Ich denke, das legt das Layout für die Unterkomponenten des rechtes Feld fest.
        ...
        pack(); // Automatically resize GUI to fit components. Not necesaraly needed but ill leave it in just in case i decide to change the size of something later. 
    ...
    private void jRadioButton1_VoiceRecognitionActionPerformed(ActionEvent evt) {
        ...
        if(myVars.isVoiceToggleSelected()){
            if(myVars.isGerToEngSelected()){
                ...
                if (inputText.toLowerCase().contains(vocab.getTranslation())) {
        ...
        // Verhindert, dass man 2 Mal auf den Kippschalter klicken muss, um den Spracherkennungsvorgang erneut zu starten. Dies setzt den Toggle-Status zurück auf "nicht markiert".
        if (myVars.isVoiceToggleSelected()) {
